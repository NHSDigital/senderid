...
batch_queue = Queue()  # Queue to hold pre-loaded batches
...
def process_batch_elements(batch, org_count, base_output_directory, base_output_directory_json):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:  # Adjust thread count for efficiency
        futures = [executor.submit(generate_organisation_yaml, elem, org_count - len(batch) + index + 1, batch_yaml_data, base_output_directory)
...
def write_summary_files(base_output_directory, base_output_directory_json):
...
def monitor_resources(interval=5):
    ...
    while not stop_flag.is_set():
        ...
        # Log resource usage
        print(f"[Resource Monitor] CPU Usage: {cpu_usage:.2f}%")
        print(f"[Resource Monitor] Memory Usage: {memory_info.rss / (1024 * 1024):.2f} MB (RSS)")
        print(f"[Resource Monitor] Disk Read: {io_counters.read_bytes / (1024 * 1024):.2f} MB")
        print(f"[Resource Monitor] Disk Write: {io_counters.write_bytes / (1024 * 1024):.2f} MB")
...
def process_batches_from_queue(base_output_directory, base_output_directory_json):
    ...
    while not stop_flag.is_set():
        ...
        process_batch_elements(batch, org_count, base_output_directory, base_output_directory_json)
...
# Main function
def main(xml_file_path, base_output_path, batch_size):
    ...
    os.makedirs(base_output_directory_json, exist_ok=True)
    ...
    monitoring_thread = Thread(target=monitor_resources, args=(5,))  # Log every 5 seconds
    ...
    try:
        ...
        process_batches_from_queue(base_output_directory, base_output_directory_json)
    ...
    write_summary_files(base_output_directory, base_output_directory_json)
...
if __name__ == "__main__":
    # Default values for XML file path, base output path, and batch size
    ...
    main(xml_file_path, base_output_path, batch_size)